...
func TestJwtAuth(t *testing.T) {
	t.Run("no fullName", func(t *testing.T) {
		adminFullName := ""
		adminID := lfutils.GenerateRandomString(10)
		adminLevel := "Developer"
		expirationDate := time.Now().Add(time.Minute * 1)
		jwt, httpStatus, err := GenerateJwt(adminFullName, adminID, adminLevel, &expirationDate)
	...
	t.Run("successfully encode JWT, make claim, and Verify JWT and compare values against claim", func(t *testing.T) {
		...
		assert.True(t, lfutils.AreSameSecond(&adminClaim.ExpirationDate, &expirationDate))
	})
}
